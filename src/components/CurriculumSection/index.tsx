    ...
    <div className="min-h-screen bg-gradient-to-br from-gray-50 to-blue-50 py-16 px-4">
     ...
     <div className="grid grid-cols-1 md:grid-cols-2 gap-8 mb-5">
          ...
          <div key={index} className="relative group">
            <div className="h-full py-8 ">
              ...
              <div className="relative p-6 text-secondary">
                <h3 className="text-[28px] font-semibold mb-4 text-white bg-primary p-2 rounded-lg pl-4">{year.title}</h3>
                <div className="space-y-6">
                  {year.semesters.map((semester) => (
                    <div key={semester.number} className=" border-primary pl-4">
                      <h4 className="text-2xl font-semibold mb-3 text-gray-400">
                        Semester {semester.number}
                      </h4>
                      <ul className="space-y-2 ">
                        {semester.details.map((detail, idx) => (
                          <li key={idx} className="flex items-start gap-2  p-2">
                            <Check className="w-8 h-8 mt-0.5 flex-shrink-0 text-primary font-bold" />
                            <span className="text-gray-900 font-semibold text-xl">{detail}</span>
                          </li>
                        ))}
                      </ul>
                    </div>
      ...
      <div className='bg-secondary p-4 rounded-md'>
        ...
        <div>
        <p className='text-3xl text-white font-semibold mt-2'>Capstone Project</p>
        <p className='text-gray-100 text-lg mt-2'>Work on real-time investment deals and financial analysis with top firms. </p>
